...
void MAXreg_wr(BYTE reg, BYTE val)
{
	...
	XSpi_SetSlaveSelect(&SpiInstance, 1u);
	Status = XSpi_Transfer(&SpiInstance, SendBuf, RecvBuf, 2);
	if (Status != XST_SUCCESS)
	{
		xil_printf ("SPI device failed single host register write %d", Status);
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0u);
}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data)
{
	...
	memset(RecvBuf, 0, sizeof(RecvBuf));
	SendBuf[0] = reg | 0x02; // cmd byte
	for (BYTE i = 1; i < nbytes + 1; i++)
	{
		SendBuf[i] = data[i-1];
	}
	XSpi_Transfer(&SpiInstance, SendBuf, RecvBuf, nbytes + 1);
	XSpi_SetSlaveSelect(&SpiInstance, 0u);
	return data + nbytes;
}
...
BYTE MAXreg_rd(BYTE reg)
{
	...
	if (Status != XST_SUCCESS)
	{
		xil_printf ("SPI device failed single host register read %d", Status);
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0u);
	return RecvBuf[1];
}
